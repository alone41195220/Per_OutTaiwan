...
const Turntable = defineComponent({
  ...
  setup() {
    ...
    const spin = () => {
      ...
      if (window.threeBg) {
        // @ts-ignore
        window.threeBg.targetSpeed = 8.0; 
      }
    ...
    const updateSpin = () => {
      ...
      if (window.threeBg) {
        // @ts-ignore
        window.threeBg.currentSpeed = 1.0 + (velocity * 15);
      }
      ...
      if (velocity > stopThreshold) {
        ...
        if (window.threeBg) {
            // @ts-ignore
            window.threeBg.targetSpeed = 1.0;
        }
        determineResult();
      }
    };
    ...
    onMounted(() => {
      ...
      nextTick(() => {
        const response = fetch(`/announcements.json?t=${Date.now()}`)
          .then(res => res.json())
