import{c as q,d as D,L as _,o as F,n as h,f as G,r,b as W}from"./index-D5Q_20HY.js";const H=D({name:"Turntable",components:{LayoutComponent:_},setup(){const C=r(!0),M=r({show:!1,message:""}),x=r(null),S=r(null),u=r(!1),s=r([{text:"阿寶Pay 100萬",color:"#fbbf24",level:1},{text:"阿寶Pay 10萬",color:"#94a3b8",level:2},{text:"阿寶Pay 1萬",color:"#d97706",level:3},{text:"銘謝惠顧",color:"#f1f5f9",level:0},{text:"銘謝惠顧",color:"#e2e8f0",level:0},{text:"銘謝惠顧",color:"#cbd5e1",level:0}]),v=r(""),p=r(0),f=r(null),z=r(!1);let g=0,d=0;const I=.988,B=5e-4,j=(t,e)=>{let o=parseInt(t.slice(1,3),16)+e,l=parseInt(t.slice(3,5),16)+e,a=parseInt(t.slice(5,7),16)+e;return o=Math.max(0,Math.min(255,o)),l=Math.max(0,Math.min(255,l)),a=Math.max(0,Math.min(255,a)),`#${o.toString(16).padStart(2,"0")}${l.toString(16).padStart(2,"0")}${a.toString(16).padStart(2,"0")}`},c=()=>{const t=x.value;if(!t)return;const e=t.getContext("2d",{alpha:!0});if(!e)return;const o=document.documentElement.classList.contains("dark"),l=t.width/2,a=t.height/2,n=Math.min(l,a)-20,i=Math.PI*2/s.value.length;e.clearRect(0,0,t.width,t.height),e.save(),e.beginPath(),e.arc(l,a,n+5,0,Math.PI*2),e.shadowBlur=30,e.shadowColor=o?"rgba(96, 165, 250, 0.3)":"rgba(0, 0, 0, 0.1)",e.fillStyle=o?"#0f172a":"#ffffff",e.fill(),e.restore(),s.value.forEach((b,$)=>{const k=g+$*i,E=k+i;e.save(),e.beginPath(),e.moveTo(l,a),e.arc(l,a,n,k,E);const y=e.createRadialGradient(l,a,0,l,a,n);y.addColorStop(0,b.color),y.addColorStop(1,j(b.color,-15)),e.fillStyle=y,e.fill(),e.strokeStyle=o?"rgba(255,255,255,0.1)":"rgba(255,255,255,0.4)",e.lineWidth=1.5,e.stroke(),e.save(),e.translate(l,a),e.rotate(k+i/2),e.textAlign="right",b.level===1?(e.shadowBlur=8,e.shadowColor="#fff",e.fillStyle="#92400e"):e.fillStyle=o?"#f1f5f9":"#334155",e.font=`bold ${Math.max(14,n*.08)}px "Inter", system-ui, sans-serif`,e.fillText(b.text,n-25,6),e.restore(),e.restore()}),e.save(),e.beginPath(),e.arc(l,a,28,0,Math.PI*2);const m=e.createLinearGradient(l-20,a-20,l+20,a+20);m.addColorStop(0,o?"#334155":"#f8fafc"),m.addColorStop(1,o?"#0f172a":"#cbd5e1"),e.fillStyle=m,e.shadowBlur=15,e.shadowColor="rgba(0,0,0,0.4)",e.fill(),e.beginPath(),e.arc(l,a,6,0,Math.PI*2),e.fillStyle="#10b981",e.fill(),e.restore()},L=()=>{u.value||(f.value=null,u.value=!0,d=Math.random()*.45+.35,window.threeBg&&(window.threeBg.targetSpeed=8),requestAnimationFrame(P))},P=()=>{g+=d,d*=I,window.threeBg&&(window.threeBg.currentSpeed=1+d*15),c(),d>B?requestAnimationFrame(P):(u.value=!1,d=0,window.threeBg&&(window.threeBg.targetSpeed=1),R())},R=()=>{const t=Math.PI*2/s.value.length,e=(g%(Math.PI*2)+Math.PI*2)%(Math.PI*2),o=Math.PI*1.5;let l=-1;for(let a=0;a<s.value.length;a++){const n=(e+a*t)%(Math.PI*2),i=(n+t)%(Math.PI*2);if(n<i){if(o>=n&&o<=i){l=a;break}}else if(o>=n||o<=i){l=a;break}}l!==-1&&(f.value=s.value[l],z.value=!0,f.value.level===1&&(W(window.innerWidth/2,window.innerHeight/2,"#fbbf24"),window.threeBg&&typeof window.threeBg.celebrate=="function"&&window.threeBg.celebrate()))},T=()=>{if(!v.value)return;const t=["#f87171","#fb923c","#fbbf24","#34d399","#60a5fa","#818cf8","#a78bfa","#f472b6"],e=t[Math.floor(Math.random()*t.length)];s.value.push({text:v.value,color:e,level:p.value}),v.value="",p.value=0,h(c)},A=t=>{s.value.length<=2||(s.value.splice(t,1),h(c))},O=()=>{s.value=[{text:"阿寶Pay 100萬",color:"#fbbf24",level:1},{text:"阿寶Pay 10萬",color:"#94a3b8",level:2},{text:"阿寶Pay 1萬",color:"#d97706",level:3},{text:"銘謝惠顧",color:"#f1f5f9",level:0},{text:"銘謝惠顧",color:"#e2e8f0",level:0},{text:"銘謝惠顧",color:"#cbd5e1",level:0}],h(c)},w=()=>{const t=x.value,e=S.value;if(!t||!e)return;const o=Math.min(e.clientWidth,500);t.width=o,t.height=o,c()};return F(()=>{w(),window.addEventListener("resize",w),new MutationObserver(()=>{c()}).observe(document.documentElement,{attributes:!0,attributeFilter:["class"]}),h(()=>{fetch(`/Per_OutTaiwan/announcements.json?t=${Date.now()}`).then(e=>e.json()).then(e=>{e.turntable&&(M.value=e.turntable)}).catch(e=>console.error(e))})}),G(()=>{window.removeEventListener("resize",w)}),{canvasRef:x,containerRef:S,isSpinning:u,prizes:s,newItemText:v,newItemLevel:p,result:f,showResultModal:z,isSettingsOpen:C,announcement:M,spin:L,addPrize:T,removePrize:A,resetToDefault:O}},template:`<LayoutComponent title="幸運轉盤">
